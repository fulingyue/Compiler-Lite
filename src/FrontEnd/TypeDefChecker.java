...
//        check();
    ...
    public void visit(UnaryOpNode node) throws Exception {
        super.visit(node);
        switch (node.getOp()) {
            case PREFIX_DEX: case PREFIX_INC:
    ...
    public void visit(BinaryOpNode node) throws Exception {
        ...
        switch (node.getOp()) {
            ...
            node.setExprType(lhs.getExprType());
                break;
            case OR: case AND: case XOR: case DIV:case MOD:case MUL:case SUB:case LSHIFT:case RSHIFT:
                // ^ & | / % * - << >>
            ...
            case LAND:case LOR:// && ||
                ...
                node.setExprType(new PrimitiveTypeNode("bool"));
                break;
            case GE:case GT: case LE:case LT: //LT : '<'; GT : '>'; LE : '<='; GE : '>=';
                if(!lhs.getExprType().equalTo(rhs.getExprType()))
...
//                    String name = defNode.getClassName() + "_" + method.getReferenceName();
    ...
    public void visit(FunctionCallNode node) throws Exception {
        ...
        if(node.getParent() instanceof MemberAccessNode){
            ClassDefNode classdef = ((MemberAccessNode) node.getParent()).getClassDefNode();
            def = classdef.getScope().getFunction(name);
            if(def == null)
                throw new SemanticException(node.getCaller().getLocation(),"class" + classdef.getClassName() + "does not have such method");
        } else {
            def = topLevelScope.getFunctionInScope(name);
            if(def == null)
                throw new SemanticException(node.getCaller().getLocation(), "no such function");
        }
